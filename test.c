...
//         exit(0);
//     }
//     vars->key_pressed_time = time(NULL);
...
//     if (current_time - vars->key_pressed_time > 2)
// Check if key was pressed for more than 2 seconds
//     {
...
void	render_rainbow(t_data *data)
{
	...
	mlx_put_image_to_window(data->mlx, data->win, data->textures, 0, 0);
}
...
//     for (int y = 0; y < win_height; y += vars->textures_height)
//     {
//         for (int x = 0; x < win_width; x += vars->textures_width)
//         {
//             mlx_put_image_to_window(vars->mlx, vars->win, vars->textures, x,
	y);
	//         }
	//     }
	// }
	...
	//     vars.textures = mlx_xpm_file_to_image(vars.mlx, relative_path,
		&vars.textures_width, &vars.textures_height);
		//     if (!vars.textures)
		//     {
		...
		//     vars.win = mlx_new_window(vars.mlx, win_width, win_height,
		"Tiled Texture");
		//     if (!vars.win)
		//     {
		//         printf("Error: Failed to create window\n");

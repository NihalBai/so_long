...
void	render_player(t_data *game)
{
	...
	current_texture = game->player.textures[game->player.direction]
...
void	process_movement(int keycode, t_data *game, int *player_moved)
{
	...
	if (keycode == 119 || keycode == 65362)
		move_player(game, game->player.x, game->player.y - 1, 3);
	else if (keycode == 115 || keycode == 65364)
		move_player(game, game->player.x, game->player.y + 1, 0);
	else if (keycode == 97 || keycode == 65361)
		move_player(game, game->player.x - 1, game->player.y, 1);
	else if (keycode == 100 || keycode == 65363)
		move_player(game, game->player.x + 1, game->player.y, 2);
...
void	check_game_status(t_data *game, int player_moved)
{
	...
	{
		game->game_lost = 1;
		game->map_data[game->player.y][game->player.x] = 'L';
